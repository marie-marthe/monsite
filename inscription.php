<?php
    ...
    include('bd/connexionDB.php'); // Fichier PHP contenant la connexion � votre BDD
    ...
    if(!empty($_POST)){
        ...
        if (isset($_POST['inscription'])){
            ...
            if(empty($nom)){
                $valid = false;
                $er_nom = ("Le nom d' utilisateur ne peut pas etre vide");
            }       

            //  V�rification du pr�nom
            if(empty($prenom)){
                $valid = false;
                $er_prenom = ("Le prenom d' utilisateur ne peut pas etre vide");
            }       

            // V�rification du mail
            if(empty($mail)){
                $valid = false;
                $er_mail = "Le mail ne peut pas etre vide";

                // On verifit que le mail est dans le bon format
            }elseif(!preg_match("/^[a-z0-9\-_.]+@[a-z]+\.[a-z]{2,3}$/i", $mail)){
                $valid = false;
                $er_mail = "Le mail n'est pas valide";

            }else{
                // On verifit que le mail est disponible
...
<html lang="fr">
    ...
    <body>      
        ...
        <form method="post">
            ...
            ?>
            <input type="text" placeholder="Votre prenom" name="prenom" value="<?php if(isset($prenom)){ echo $prenom; }?>" required>
