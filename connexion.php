  ...
  // S'il y a une session alors on ne retourne plus sur cette page  
    ...
    if(!empty($_POST)){
        ...
        if (isset($_POST['connexion'])){
            ...
            // Si on a pas de r�sultat alors c'est qu'il n'y a pas d'utilisateur correspondant au couple mail / mot de passe
	    if (!isset($req['id'])){
        	$valid = false;
	        $er_mail = "Le mail ou le mot de passe est incorrecte";

	    }elseif($req['n_mdp'] == 1){ // On remet � z�ro la demande de nouveau mot de passe s'il y a bien un couple mail / mot de passe
